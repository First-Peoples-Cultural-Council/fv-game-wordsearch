...
class Main {
  ...
  getHighlightColour() {
    this.highlightIndex =
      (this.highlightIndex + 1) % (this.highlightTints.length - 1)
  ...
  mapWords() {
    ...
    words.forEach((word) => {
      ...
      letters = letters.concat(word.characters)
    })
  ...
  preload() {
    ...
    for (let i = 0; i < letterCount; i++) {
      ...
      context.fillText(
        letter,
        tileImageBlockWidth + tileImageBlockWidth / 2,
        tileImage.height / 2
      )
  ...
  create() {
    ...
    this.solution.forEach((entry) => {
      ...
      let translatedText = this.make.text(0, 30, `${translation}`, {
  ...
  createWordFeature(entry) {
    ...
    const word = this.make.text(30, 210, wordData.word, {
    ...
    const translation = this.make.text(0, 240, wordTranslation, {
      wordWrapWidth: 200,
